...
async function createUser(email, role, department) {
    const password = await bcryptjs_1.default.hash(DEFAULT_PASSWORD, 10);
...
async function createDocumentWithChunks(document) {
    const uploader = await prisma_1.prisma.user.findUnique({ where: { email: document.uploadedByEmail } });
    if (!uploader) {
        throw new Error(`Uploader not found for ${document.filename}`);
    }
    const viewers = await prisma_1.prisma.user.findMany({
        where: { email: { in: document.visibleToEmails } },
        select: { id: true },
    });
    const visibleToUserIds = viewers.map((viewer) => viewer.id);
    const createdDocument = await prisma_1.prisma.document.create({
        data: {
    ...
    for (let index = 0; index < chunks.length; index += 1) {
        ...
        const embedding = await gemini_1.embeddingModel.embedContent(chunk);
...
async function resetDatabase() {
    await prisma_1.prisma.approvedAnswer.deleteMany();
    await prisma_1.prisma.workflowDraft.deleteMany();
