...
const NO_MATCH_RESPONSE = {
    ...
    content: "The requested information is not present in the currently accessible company documents. I can help with something else if you want to ask about another policy, process, document summary, or department-specific topic.",
    actionItems: [
...
function buildFallbackTitle(queryText) {
...
const executeQuery = async (queryText, user) => {
    ...
    if (scoredDocs.length === 0 || scoredDocs[0].score < 0.15) {
        return prisma_1.prisma.workflowDraft.create({
            data: {
                userId: user.userId,
                query: queryText,
                sourceChunks: [],
                draftType: NO_MATCH_RESPONSE.draftType,
                draftContent: NO_MATCH_RESPONSE,
                status: "PENDING",
            },
        });
    }
